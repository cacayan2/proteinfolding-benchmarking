    ...
    for p in [ur30, bfd, blastmat, pdb100]:
        ...
        def append_export(name, value):
            ...
            if line not in existing: # if not already in bashrc
                ...
                print(f"[TOOLS] bashrc already contains {name}")
...
def rfaa_dependencies():
    ...
    run(f"python3 {cfg.RF_DIR}/rf2aa/SE3Transformer/setup.py install")

    # Prompting user to download signalp6.
    signalp_dir = os.path.join(cfg.TOOLS_DIR, "signalp6")
    os.makedirs(signalp_dir, exist_ok = True)

    tarball_path = os.path.join(signalp_dir, "signalp-6.0h.fast.tar.gz")
    ...
    conda_prefix = subprocess.check_output(
        f"conda run -n {cfg.RF_ENV} bash -c 'echo $CONDA_PREFIX'",
        shell=True
    ).decode().strip()
    ...
    if not os.path.exists(distilled) and not os.path.exists(ensemble):
        print("[TOOLS] Running signalp6-register...")
        run(f"conda run -n {cfg.RF_ENV} signalp6-register {tarball_path}")
    # Rename model files (instructed by documentation)
    os.rename(distilled, ensemble)
    if os.path.exists(distilled):
        print(f"[TOOLS] Distilled weights found at:\n   {distilled}")
    if os.path.exists(ensemble):
        print(f"[TOOLS] Ensemble weights found at:\n   {ensemble}")
    ...
    subprocess.run(f"bash {cfg.RF_DIR}/install_dependencies.sh", cwd = cfg.RF_DIR, shell = True)
    ...
    if not os.path.exists(f"{cfg.RF_DIR}/UniRef30_2020_06"):
        ...
        run(f"tar -xvzf UniRef30_2020_06_hhsuite.tar.gz -C {cfg.RF_DIR}")
    ...
    if not os.path.exists(f"{cfg.RF_DIR}/pdb100_2021Mar03"):
        run(f"mkdir -p {cfg.RF_DIR}/pdb100_2021Mar03")
    ...
    if not os.path.exists(f"{cfg.RF_DIR}/blast-2.2.26"):
        ...
        run(f"aria2c -x 16 -s 16 --dir={cfg.RF_DIR} https://ftp.ncbi.nlm.nih.gov/blast/executables/legacy.NOTSUPPORTED/2.2.26/blast-2.2.26-x64-linux.tar.gz")
        run(f"tar -xvzf {cfg.RF_DIR}/blast-2.2.26-x64-linux.tar.gz -C {cfg.RF_DIR}/blast-2.2.26")
...
def prepare_diffdock_weights():
    ...
    if has_diffdock_weights():
        print("[TOOLS] DiffDock weights already installed, not preparing weights.")
        return
    print("[TOOLS] Preparing DiffDock weights...")
    # This is a file that only the author of this pipeline has - it automatically downloads the correct zip file.
    if os.path.exists(f"{cfg.PROJECT_ROOT}/scripts/diffdock_weights.py") and not os.path.exists(f"{cfg.DD_DIR}/diffdock_weights.zip"):
        print("[TOOLS] Downloading DiffDock weights...")
        subprocess.run(f"python {cfg.PROJECT_ROOT}/scripts/diffdock_weights.py", shell = True)
    # Unzips weights.
    MODEL_DIR = os.path.join(cfg.DD_DIR, "models")
    ZIP_DIR = os.path.join(cfg.DD_DIR, "diffdock_weights.zip")
    subprocess.run(f"unzip {ZIP_DIR} 'diffdock-main/workdir/*' -d {cfg.DD_DIR}/tmp_dd", shell = True)
    subprocess.run(f"mv {cfg.DD_DIR}/tmp_dd/diffdock-main/workdir/* {MODEL_DIR}/", shell = True)
    subprocess.run("rm -rf tmp_dd diffdock_weights.zip", shell = True, cwd = cfg.DD_DIR)

def has_diffdock_weights():
    """
    Checks if DiffDock weights are installed.
    """
    if os.path.exists(f"{cfg.DD_DIR}/models/paper_confidence_model") and os.path.exists(f"{cfg.DD_DIR}/models/paper_score_model"):
        return True
    else:
        return False
